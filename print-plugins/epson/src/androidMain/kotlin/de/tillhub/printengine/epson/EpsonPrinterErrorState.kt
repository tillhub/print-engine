...
internal object EpsonPrinterErrorState {
    ...
    private fun evaluateStatusInfo(status: PrinterStatusInfo): PrinterState = when {
    ...
    private fun handleUnrecoverableError(status: PrinterStatusInfo): PrinterState = when (status.unrecoverError) {
        Printer.LOW_VOLTAGE_ERR -> PrinterState.Error.VoltageTooLow
        else -> PrinterState.Error.Malfunctions
    }

    private fun handleAutoRecoverableError(status: PrinterStatusInfo): PrinterState = when (status.autoRecoverError) {
