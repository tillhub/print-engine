...
class ExternalPrinterManagerImplTest {
    ...
    fun `discoverExternalPrinters emits Idle then Discovered with unique printers`() = runTest {
        ...
        everySuspend { epsonPrinterDiscovery.observePrinters } returns
            flow {
                emit(DiscoveryState.Discovering(listOf(printer1)))
            }

        everySuspend { starPrinterDiscovery.observePrinters } returns
            flow {
        ...
        val result =
            manager
                .discoverExternalPrinters(epsonPrinterDiscovery, starPrinterDiscovery)
                .take(3)
                .toList()
    ...
    fun `discoverExternalPrinters handles single discovery correctly`() = runTest {
        ...
        val result =
            manager
                .discoverExternalPrinters(epsonPrinterDiscovery)
                .take(2)
                .toList()
